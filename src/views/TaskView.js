...
class TaskView {
    ...
    _handleTaskDelete(taskId) {
        ...
        if (
            confirm(`Apakah Anda yakin ingin menghapus task "${task.title}"?`)
        ) {
    ...
    _createTaskHTML(task) {
        ...
        if (task.assigneeId) {
            const userResponse = this.userController.getUserById(
                task.assigneeId
            );
            if (userResponse.success) {
                assigneeName =
                    userResponse.data.fullName || userResponse.data.username;
            }
        }
